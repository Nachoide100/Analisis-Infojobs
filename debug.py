...
print("🕵️‍♂️ Inspeccionando estructura JSON...")

try:
    response = requests.get(url, headers=headers, params=params)
    data = response.json()
    
    # Buscamos la lista de ofertas
    items = data.get('items', []) or data.get('offers', [])
    
    if items:
        primer_oferta = items[0]
        print("\n✅ ¡Encontrado! Aquí están las claves de la primera oferta:")
        print(json.dumps(primer_oferta, indent=4, ensure_ascii=False))
    else:
        print("❌ No se encontraron items. Revisa las Cookies.")
