...
PAGINAS_A_SCRAPEAR = 5  # Empieza con 5 para probar, luego sube a 20 o 30
FILENAME = "ofertas_infojobs_raw.csv"

# URL base limpia (sin parámetros)
url = "https://www.infojobs.net/webapp/offers/search"

# --- TUS HEADERS (Convertidos de tu cURL) ---
# He limpiado y estructurado lo que me has pasado
headers = {
...
print("🚀 Iniciando scraping modo 'Ninja' en InfoJobs...")
...
for page in range(1, PAGINAS_A_SCRAPEAR + 1):
    ...
    params = {
        'keyword': KEYWORD,
        'normalizedJobTitleIds': '2511_d3edb8f8-3a06-47a0-8fb9-9b212c006aa2',
        'segmentId': '',
        'page': page,
        'sortBy': 'RELEVANCE',
    ...
    try:
        # Hacemos la petición fingiendo ser el navegador
        response = requests.get(url, headers=headers, params=params)
        
        if response.status_code == 200:
            data = response.json()
            
            # InfoJobs suele devolver las ofertas en 'items' o 'offers'
            # Vamos a buscar donde están
            items = data.get('items', []) or data.get('offers', [])
            
            if not items:
                print("⚠️ No se encontraron más ofertas o se acabó la lista.")
                break
            ...
            for item in items:
                ...
                offer = {
                    ...
                    'city': item.get('city'),
                    'salaryMin': item.get('salaryMin', {}).get('value'),
                    'salaryMax': item.get('salaryMax', {}).get('value'),
                    'salaryDescription': item.get('salaryDescription'),
                    'link': item.get('link'),
            ...
            print("❌ ERROR 403: Nos han detectado o las cookies han caducado.")
            print("👉 SOLUCIÓN: Vuelve a la web, recarga, copia el cURL nuevo y actualiza la variable 'headers'.")
            break
        else:
    ...
    # Pausa humana (CRUCIAL para no ser bloqueado)
...
if all_offers:
    ...
    print(f"\n🎉 ¡Éxito! Se han guardado {len(df)} ofertas en '{FILENAME}'")
    print(df.head())
else:
    print("\n☹️ No se ha podido extraer ninguna oferta.")